...
const STORAGE_ORDERS = 'zsmk_sched_orders_v1';
const STORAGE_ROUTINGS = 'zsmk_sched_routings_v1';
const STORAGE_STATIONS = 'zsmk_sched_stations_v1';
const STORAGE_OVERRIDES = 'zsmk_sched_overrides_v1';

// Базовые станки завода с типовыми графиками (Лазер 2/2 12ч, Склейка 5/2 8ч)
...
// Типовой технологический маршрут: Лазер -> Гибочник -> Склейка -> Сборка
export const DEFAULT_ROUTINGS: Record<string, ProductRouting> = {
  pr1: {
    ...
    steps: [
      ...
      { stepNumber: 30, professionId: 'p2', professionName: 'Холодная склейка', normPerUnit: 0.718, setupTimeHours: 1.0, bufferHoursAfter: 2 },
...
export const DEFAULT_ORDERS: ProductionOrder[] = [
  { id: 'ord_1', orderNumber: 'ЗК-104', customer: 'Инкерман', productId: 'pr1', productName: 'Сотовые панели Кемерово', quantity: 45, unit: 'м²', dueDate: '2026-09-22', priority: 'urgent', status: 'new' },
  { id: 'ord_2', orderNumber: 'ЗК-105', customer: 'Кемерово-Фасад', productId: 'pr1', productName: 'Сотовые панели Кемерово', quantity: 70, unit: 'м²', dueDate: '2026-09-25', priority: 'normal', status: 'new' }
];
  ...
  async loadData() {
    ...
    if (isSupabaseConfigured() && supabase) {
      try {
        ...
        console.warn('Supabase недоступен, загружаем локальный кэш:', err);
      }
    }

    // Резервное чтение из LocalStorage
    const localOrd = localStorage.getItem(STORAGE_ORDERS);
    if (localOrd && (!orders || orders.length === 0)) orders = JSON.parse(localOrd);

...
export function renderCharts(calc: CalculationResult, data: ScenarioData) {
  ...
  const ctxStaff = (document.getElementById("chartStaffMonthly") as HTMLCanvasElement)?.getContext("2d");
  if (ctxStaff) {
  ...
  const ctxProd = (document.getElementById("chartProductsShare") as HTMLCanvasElement)?.getContext("2d");
  if (ctxProd) {
    productsChartInstance = new Chart(ctxProd, {
      ...
      options: {
        ...
        plugins: {
          legend: {
            position: 'right', // ❗️ ЛЕГЕНДА СПРАВА ДЕЛАЕТ ДИАГРАММУ КРУПНОЙ
...
### 4. `workforce/src/ui/normingTab.ts` (Исправление записи в реестр и параметров)

Замените `workforce/src/ui/normingTab.ts`:
...
export function setupNormingTab(
  ...
  const pushHandler = () => {
    ...
    if (prod && prof) {
      ...
      recalc();
      // МГНОВЕННО ОБНОВЛЯЕМ ТАБЛИЦУ РЕЕСТРА В DOM
      renderSavedNormsRegistry(data, onConfigDeleted || (() => {}));
    }
  };

  document.getElementById("btnPushStatNormToRouting")?.addEventListener("click", pushHandler);
  document.getElementById("btnPushChronoNormToRouting")?.addEventListener("click", pushHandler);

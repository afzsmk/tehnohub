...
import type { CalendarDay, DowntimeEvent, Employee, EmployeeSchedule, Equipment, EquipmentBlock, MaintenanceOrder, MesState, ProductionEvent, ProductionOrder, ProductionResult, ProductionTask, Product, QualityInspection, QualityStatus, RouteOperation, ShiftDefinition } from '../types';
...
function buildRoutes(routeRows: Array<Record<string, unknown>>): Map<string, RouteOperation[]> {
  ...
  for (const row of routeRows) {
    ...
    const operation: RouteOperation = {
      id: str(row.id), sequence: num(row.sequence), code: str(row.code), name: str(row.name),
      ...
      setupMinutes: num(row.setup_minutes), runMinutesPerUnit: num(row.run_minutes_per_unit)
    };
...
function fallbackRoutesByOrder(tasks: ProductionTask[]): Map<string, RouteOperation[]> {
  ...
  for (const task of tasks) {
    ...
    if (!route.some(operation => operation.id === task.operationId)) {
      route.push({
        id: task.operationId, sequence: task.operationSequence, code: task.operationId, name: task.operationId,
        workCenter: '—', setupMinutes: 0,
...
export async function loadMesStateFromSupabase(client: SupabaseClient, fallback: MesState): Promise<RemoteSnapshot> {
  const [plans, productsRows, employeeRows, equipmentRows, shiftRows, calendarRows, scheduleRows, equipmentBlockRows, orderRows, taskRows, assignmentRows, downtimeRows, maintenanceRows, resultRows, qualityInspectionRows, eventRows, routeRows] = await Promise.all([
    selectAll(client, 'operational_plans'), selectAll(client, 'products'), selectAll(client, 'employees'), selectAll(client, 'equipment'),
    selectAll(client, 'shift_definitions'), selectAll(client, 'calendar_days'), selectAll(client, 'employee_schedules'), selectAll(client, 'equipment_blocks'),
    ...
    selectAll(client, 'maintenance_orders'), selectAll(client, 'production_results'), selectAll(client, 'quality_inspections'), selectAll(client, 'production_events'),
    selectAll(client, 'route_operations')
  ]);
  ...
  const tasks: ProductionTask[] = taskRows.map(row => ({
    id: str(row.id), orderId: str(row.order_id), operationId: str(row.operation_id), operationSequence: num(row.operation_sequence),
    status: str(row.status) as ProductionTask['status'], plannedStart: str(row.planned_start), plannedEnd: str(row.planned_end),
    actualStart: row.actual_start ? str(row.actual_start) : undefined, actualEnd: row.actual_end ? str(row.actual_end) : undefined,
    plannedQuantity: num(row.planned_quantity), actualQuantity: num(row.actual_quantity), assignedEmployeeIds: [], assignedEquipmentIds: [],
    qualityRequired: bool(row.quality_required), qualityStatus: row.quality_status == null ? undefined : str(row.quality_status) as QualityStatus,
  ...
  for (const row of assignmentRows) {
    ...
    const current = assignmentByTask.get(taskId) ?? { employeeIds: [], equipmentIds: [] };
    const employeeId = str(row.employee_id);
    const equipmentId = str(row.equipment_id);
  ...
  for (const task of tasks) {
    ...
    if (assignment) {
      task.assignedEmployeeIds = assignment.employeeIds;
      task.assignedEquipmentIds = assignment.equipmentIds;
    }
  }
  ...
  const orders: ProductionOrder[] = orderRows.map(row => ({
    id: str(row.id), externalId: row.external_id ? str(row.external_id) : undefined, number: str(row.number), productId: str(row.product_id),
    quantity: num(row.quantity), completedQuantity: num(row.completed_quantity), dueAt: str(row.due_at),
    priority: str(row.priority) as ProductionOrder['priority'], status: str(row.status) as ProductionOrder['status'],
  ...
  return {
    plan: latestPlan
      ? {
          id: str(latestPlan.id), version: num(latestPlan.version, 1),
          horizonStart: str(latestPlan.horizon_start, fallback.plan.horizonStart),
          horizonEnd: str(latestPlan.horizon_end, fallback.plan.horizonEnd),
          status: str(latestPlan.status) as MesState['plan']['status'],
          sourcePlanId: latestPlan.source_plan_id ? str(latestPlan.source_plan_id) : undefined,
          sourcePlanVersion: latestPlan.source_plan_version ? num(latestPlan.source_plan_version) : undefined
        }
      : fallback.plan,
    ...
    equipmentBlocks: equipmentBlockRows.length ? equipmentBlockRows.map(row => ({ id: str(row.id), equipmentId: str(row.equipment_id), start: str(row.start_at), end: str(row.end_at), reason: str(row.reason) as EquipmentBlock['reason'], comment: row.comment ? str(row.comment) : undefined })) : fallback.equipmentBlocks,
    orders,
    tasks,
    ...
    qualityInspections: qualityInspectionRows.map(row => ({
      id: str(row.id), taskId: str(row.task_id), inspectedAt: str(row.inspected_at), inspectorId: str(row.inspector_id),
      status: str(row.status) as QualityInspection['status'], goodQuantity: num(row.good_quantity), scrapQuantity: num(row.scrap_quantity),
      defectCode: row.defect_code ? str(row.defect_code) : undefined, comment: row.comment ? str(row.comment) : undefined
    })),

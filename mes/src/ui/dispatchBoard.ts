...
interface TaskIssue {
  kind: 'RESOURCE' | 'CALENDAR' | 'MAINTENANCE' | 'QUALIFICATION' | 'EQUIPMENT';
  message: string;
}
...
function issuesFor(
  ...
  for (const other of tasks) {
    ...
    if (task.assignedEmployeeIds.some(id => other.assignedEmployeeIds.includes(id))) issues.push({ kind: 'RESOURCE', message: 'Сотрудник одновременно назначен на другое задание.' });
    if (task.assignedEquipmentIds.some(id => other.assignedEquipmentIds.includes(id))) issues.push({ kind: 'RESOURCE', message: 'Оборудование одновременно используется другим заданием.' });
  }
...
export function renderDispatchBoard(options: DispatchBoardOptions): string {
  const { tasks, employees, equipment, shifts, calendar, equipmentBlocks, operations } = options;

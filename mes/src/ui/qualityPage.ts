...
export async function mountQualityPage(root: HTMLElement, client: SupabaseClient): Promise<void> {
  ...
  host.querySelector<HTMLFormElement>('#quality-page-form')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget as HTMLFormElement;const submit=form.querySelector<HTMLButtonElement>('button[type="submit"]');if(submit?.disabled)return;const data=new FormData(form);const status=String(data.get('status')??'APPROVED') as 'APPROVED'|'REJECTED';const good=Number(data.get('good')??0);const scrap=Number(data.get('scrap')??0);const defect=String(data.get('defect')??'').trim();if(!Number.isFinite(good)||!Number.isFinite(scrap)||good<0||scrap<0||good+scrap<=0){window.alert('Укажите корректные количества результата контроля');return;}if(status==='REJECTED'&&!defect){window.alert('Для отклонения необходимо указать код дефекта');return;}if(submit)submit.disabled=true;try{await rpc.submitQualityInspection(String(data.get('task')??''),status,good,scrap,defect,String(data.get('comment')??''),new Date().toISOString());window.location.reload();}catch(error){if(submit)submit.disabled=false;window.alert(error instanceof Error?error.message:'Не удалось сохранить решение ОТК');}});
}

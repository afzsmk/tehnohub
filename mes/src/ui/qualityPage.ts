...
export async function mountQualityPage(root: HTMLElement, client: SupabaseClient): Promise<void> {
  ...
  host.querySelectorAll<HTMLButtonElement>('[data-quality-config]').forEach(button=>button.addEventListener('click',async()=>{if(button.disabled)return;const task=tasks.find(t=>t.id===button.dataset.qualityConfig);if(!task)return;button.disabled=true;try{await rpc.setQualityRequired(task.id,!task.quality_required);window.location.reload();}catch(error){button.disabled=false;window.alert(error instanceof Error?error.message:'Не удалось изменить требование ОТК');}}));
  host.querySelectorAll<HTMLButtonElement>('[data-quality-request]').forEach(button=>button.addEventListener('click',async()=>{if(button.disabled)return;button.disabled=true;try{await rpc.requestQualityCheck(button.dataset.qualityRequest??'');window.location.reload();}catch(error){button.disabled=false;window.alert(error instanceof Error?error.message:'Не удалось отправить запрос ОТК');}}));
  host.querySelectorAll<HTMLButtonElement>('[data-quality-review]').forEach(button=>button.addEventListener('click',()=>{const form=host.querySelector<HTMLFormElement>('#quality-page-form');if(!form)return;const task=tasks.find(item=>item.id===(button.dataset.qualityReview??''));if(!task)return;(form.elements.namedItem('task') as HTMLInputElement).value=task.id;form.dataset.idempotencyKey='';(form.elements.namedItem('good') as HTMLInputElement).value=String(task.actual_quantity);(form.elements.namedItem('scrap') as HTMLInputElement).value='0';(form.elements.namedItem('defect') as HTMLInputElement).value='';(form.elements.namedItem('comment') as HTMLInputElement).value='';(form.querySelector('[data-quality-form-hint]') as HTMLElement).textContent=`Задание ${task.id}: факт ${task.actual_quantity} из ${task.planned_quantity}. При APPROVED полного факта сервер завершит задание автоматически.`;form.hidden=false;form.scrollIntoView({behavior:'smooth',block:'center'});}));
  host.querySelector<HTMLFormElement>('#quality-page-form')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget as HTMLFormElement;const submit=form.querySelector<HTMLButtonElement>('button[type="submit"]');if(submit?.disabled)return;const data=new FormData(form);const status=String(data.get('status')??'APPROVED') as 'APPROVED'|'REJECTED';const good=Number(data.get('good')??0);const scrap=Number(data.get('scrap')??0);const defect=String(data.get('defect')??'').trim();if(!Number.isFinite(good)||!Number.isFinite(scrap)||good<0||scrap<0||good+scrap<=0){window.alert('Укажите корректные количества результата контроля');return;}if(status==='REJECTED'&&!defect){window.alert('Для отклонения необходимо указать код дефекта');return;}if(submit)submit.disabled=true;const idempotencyKey=form.dataset.idempotencyKey||newIdempotencyKey();form.dataset.idempotencyKey=idempotencyKey;try{await rpc.submitQualityInspection(String(data.get('task')??''),status,good,scrap,defect,String(data.get('comment')??''),new Date().toISOString(),idempotencyKey);window.location.reload();}catch(error){if(submit)submit.disabled=false;window.alert(error instanceof Error?error.message:'Не удалось сохранить решение ОТК');}});
}

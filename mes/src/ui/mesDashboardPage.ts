...
export async function mountMesDashboardPage(root: HTMLElement, client: SupabaseClient): Promise<void> {
  ...
  const render=async():Promise<void>=>{
    try {
      const [ordersQ, tasksQ, assignmentsQ, employeesQ, downtimeQ, resultsQ, maintenanceQ, equipmentQ] = await Promise.all([
        ...
        client.from('task_assignments').select('task_id,employee_id').limit(2000),
      ...
      for(const q of [ordersQ,tasksQ,assignmentsQ,employeesQ,downtimeQ,resultsQ,maintenanceQ,equipmentQ])if(q.error)throw q.error;
      ...
      for(const item of assignments)if(!assignmentByTask.has(item.task_id))assignmentByTask.set(item.task_id,item);
      ...
      const employeeLoad=employees.map(employee=>{
        const employeeTasks=tasks.filter(task=>assignmentByTask.get(task.id)?.employee_id===employee.id&&!['CANCELLED'].includes(task.status));
        const plannedMs=employeeTasks.reduce((sum,task)=>{const start=new Date(task.planned_start).getTime();const end=new Date(task.planned_end).getTime();return sum+Math.max(0,end-start);},0);
        const activeCount=employeeTasks.filter(task=>!['COMPLETED'].includes(task.status)).length;
        return {employee,plannedHours:hours(plannedMs),activeCount};
      }).filter(item=>item.plannedHours>0).sort((a,b)=>b.plannedHours-a.plannedHours);
      ...
      const loadRows=employeeLoad.slice(0,10).map(item=>{
        const share=totalPlannedEmployeeHours>0?item.plannedHours/totalPlannedEmployeeHours*100:0;
        return `<tr><td><strong>${esc(item.employee.name)}</strong></td><td>${item.plannedHours.toFixed(1)} ч</td><td>${item.activeCount}</td><td>${share.toFixed(1)}%</td></tr>`;
      }).join('');
      ...
      host.innerHTML=`<div class="panel-head"><div><h2>Оперативный Dashboard Plan / Fact</h2><div class="subtle">Смена · производство · риски · качество · простои</div></div><button class="primary" data-dashboard-refresh>Обновить</button></div>
        ...
        <div class="dashboard-grid">
          ...
          <div class="dashboard-card"><div class="dashboard-card-head"><h3>Плановая загрузка персонала</h3><span>${totalPlannedEmployeeHours.toFixed(1)} ч</span></div><div class="subtle" style="margin-bottom:8px">Назначенное плановое время по активным заданиям; это не фактическая утилизация.</div><table><thead><tr><th>Сотрудник</th><th>План, ч</th><th>Активных заданий</th><th>Доля</th></tr></thead><tbody>${loadRows||'<tr><td colspan="4">Нет назначенных заданий</td></tr>'}</tbody></table></div>
        </div>`;
  ...
  subscribeMesRealtime(client,{tables:['production_orders','production_tasks','task_assignments','employees','production_results','downtime_events','maintenance_orders','equipment'],debounceMs:500,onChange:()=>void render()});
  await render();
}

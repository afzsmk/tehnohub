...
function statusClass(value: string): string { return ['COMPLETED'].includes(value) ? 'status-ok' : ['BLOCKED','CANCELLED','REJECTED'].includes(value) ? 'status-danger' : ['IN_EXECUTION','PARTIALLY_COMPLETED','PENDING'].includes(value) ? 'status-warning' : 'status-neutral'; }
...
export async function mountOrdersPage(root: HTMLElement, client: SupabaseClient): Promise<void> {
  ...
  host.innerHTML=`<div class="panel-head"><div><h2>Производственные заказы</h2><div class="subtle">Оперативное управление статусом, маршрутом и заданиями · роль ${esc(role)}</div></div><button class="primary" id="orders-refresh">Обновить</button></div>
    ...
    <div class="orders-table-wrap"><table><thead><tr><th>Заказ</th><th>Количество</th><th>Выполнено</th><th>Срок</th><th>Приоритет</th><th>Статус</th><th>Действия</th></tr></thead><tbody>${orders.map(o=>{
      ...
      return `<tr data-order-row="${esc(o.id)}"><td><strong>${esc(o.number)}</strong><div class="subtle">${esc(o.external_id??'')} · ${esc(o.id)}</div></td><td>${o.quantity}</td><td>${o.completed_quantity} <span class="subtle">(${pct}%)</span></td><td>${new Date(o.due_at).toLocaleDateString('ru-RU')}</td><td>${esc(o.priority)}</td><td><span class="status-pill ${statusClass(o.status)}">${statusLabel(o.status)}</span></td><td class="orders-actions">${action.join(' ')}</td></tr><tr data-order-detail-row="${esc(o.id)}" class="order-detail-row"><td colspan="7"><div class="order-detail" data-detail-host="${esc(o.id)}"></div></td></tr>`;
  ...
  const loadDetail = async (order: OrderRow, container: HTMLElement): Promise<void> => {
    container.innerHTML='<div class="subtle">Загрузка маршрута и производственных заданий…</div>';
    try {
  ...
  host.querySelectorAll<HTMLButtonElement>('[data-order-detail]').forEach(button=>button.addEventListener('click',()=>{
    ...
    detailState[open?'add':'delete'](orderId);
    row.hidden=!open;

...
interface EmployeeRow { id: string; name: string; qualification_level: number; active: boolean; }
interface EquipmentRow { id: string; name: string; work_center: string; active: boolean; }
...
function normalizeEmployee(row: Record<string, unknown>): EmployeeRow { return { id: String(row.id), name: String(row.name), qualification_level: Number(row.qualification_level), active: Boolean(row.active) }; }
function normalizeEquipment(row: Record<string, unknown>): EquipmentRow { return { id: String(row.id), name: String(row.name), work_center: String(row.work_center), active: Boolean(row.active) }; }
...
function actionButtons(task: TaskRow): string {
  ...
  if (['RUNNING', 'PARTIALLY_COMPLETED', 'PAUSED'].includes(task.status)) buttons.push(`<button class="tiny" data-task-action="BLOCK" data-task-id="${esc(task.id)}">⚠ Блок</button>`);
  if (['RUNNING', 'PARTIALLY_COMPLETED'].includes(task.status) && (!task.quality_required || task.quality_status === 'APPROVED')) buttons.push(`<button class="tiny action-complete" data-task-action="COMPLETE" data-task-id="${esc(task.id)}">✓ Завершить</button>`);
  return buttons.join(' ') || '<span class="subtle">—</span>';
}

function assignmentControls(task: TaskRow, assignment: AssignmentRow | undefined, employees: EmployeeRow[], equipment: EquipmentRow[], canAssign: boolean): string {
  if (!canAssign) return '<span class="subtle">Только просмотр</span>';
  const employeeId = assignment?.employee_id ?? '';
  const equipmentId = assignment?.equipment_id ?? '';
  const employeeOptions = employees.map(item => `<option value="${esc(item.id)}" ${item.id === employeeId ? 'selected' : ''}>${esc(item.name)} · разряд ${item.qualification_level}</option>`).join('');
  const equipmentOptions = equipment.map(item => `<option value="${esc(item.id)}" ${item.id === equipmentId ? 'selected' : ''}>${esc(item.name)} · ${esc(item.work_center)}</option>`).join('');
  return `<div class="task-resource-editor"><select data-task-employee="${esc(task.id)}" data-task-version="${task.version}" aria-label="Сотрудник"><option value="">Сотрудник не назначен</option>${employeeOptions}</select><select data-task-equipment="${esc(task.id)}" data-task-version="${task.version}" aria-label="Оборудование"><option value="">Оборудование не назначено</option>${equipmentOptions}</select><div class="subtle">Сервер проверит квалификацию, смену, пересечения и доступность.</div></div>`;
}
...
export async function mountOrdersPage(root: HTMLElement, client: SupabaseClient): Promise<void> {
  ...
  const loadDetail = async (order: OrderRow, container: HTMLElement): Promise<void> => {
    container.innerHTML = '<div class="subtle">Загрузка маршрута, заданий и ресурсов…</div>';
    try {
      ...
      const assignmentResult = tasks.length ? await client.from('task_assignments').select('task_id,employee_id,equipment_id').in('task_id', tasks.map(task => task.id)) : { data: [], error: null };
      ...
      container.querySelectorAll<HTMLSelectElement>('[data-task-employee], [data-task-equipment]').forEach(select => {
        select.addEventListener('change', async () => {
          const taskId = select.dataset.taskEmployee ?? select.dataset.taskEquipment ?? '';
          const version = Number(select.dataset.taskVersion);
          if (!taskId || !Number.isInteger(version) || version <= 0) return;
          const expectedVersion = assignmentVersions.get(taskId) ?? version;
          const dimension = select.dataset.taskEmployee ? 'employeeIds' : 'equipmentIds';
          const value = select.value;
          select.disabled = true;
          try {
            const updated = await planningRpc.assignTask(taskId, dimension === 'employeeIds' ? { employeeIds: value ? [value] : [] } : { equipmentIds: value ? [value] : [] }, expectedVersion);
            assignmentVersions.set(taskId, updated.version);
            await loadDetail(order, container);
          } catch (error) {
  ...
  host.addEventListener('click', event => {
    ...
    const actionHost = host.querySelector<HTMLElement>(`[data-task-action-host="${CSS.escape(target.dataset.taskId)}"]`);
    const orderId = (actionHost?.closest('tr') as HTMLElement | null)?.dataset.orderId;
    ...
    void (async () => {
      try {
        ...
        if (action === 'PREPARE') await planningRpc.prepareTask(target.dataset.taskId!);

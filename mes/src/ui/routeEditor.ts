...
export async function mountRouteEditor(root:HTMLElement,client:SupabaseClient):Promise<void>{
  ...
  async function load(){
    const [{data:productsData,error:productsError},{data:routesData,error:routesError},{data:wcData,error:wcError},{data:qualData,error:qualError},{data:eqData,error:eqError},{data:opsData,error:opsError}]=await Promise.all([
      ...
      client.from('equipment').select('id,code,name,active').order('code'),
    ...
    host.innerHTML=`
      ...
      ${operations.map(o=>{
        ...
        const requiredEquipmentIds=o.requiredEquipmentIds??[];
        return `<tr><td>${esc(route?`${route.code} v${route.version}`:'—')}</td><td>${esc(productById.get(o.productId)?.code??o.productId)}</td><td>${o.sequence}</td><td>${esc(o.code)}</td><td>${esc(o.name)}</td><td>${esc(wcById.get(o.workCenterId??'')?.code??o.workCenter)}</td><td>${esc(qualById.get(o.requiredQualificationId??'')?.code??String(o.requiredQualification??'—'))}</td><td>${esc(requiredEquipmentIds.map(id=>eqById.get(id)?.code??id).join(', '))}</td><td>${o.laborNormHoursPerUnit}</td><td>${o.setupNormHours}</td><td>${o.workersRequired}</td><td><button class="tiny" data-edit-route="${esc(o.id)}">Изменить</button> <button class="tiny danger-button" data-remove-route="${esc(o.id)}">Удалить</button></td></tr>`;

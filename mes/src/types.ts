export type UserRole = 'ADMIN'|'PRODUCTION_MANAGER'|'PLANNER'|'DISPATCHER'|'MASTER'|'OPERATOR'|'MAINTENANCE'|'QUALITY'|'ANALYST';
export type TaskStatus = 'DRAFT'|'PLANNED'|'ASSIGNED'|'READY'|'RUNNING'|'PAUSED'|'BLOCKED'|'PARTIALLY_COMPLETED'|'COMPLETED'|'CANCELLED';
export type OrderStatus = 'IMPORTED'|'PLANNED'|'RELEASED'|'IN_EXECUTION'|'PARTIALLY_COMPLETED'|'COMPLETED'|'CANCELLED'|'BLOCKED';
...
export interface QualityInspection { id:string; taskId:string; inspectedAt:string; inspectorId:string; status:'PENDING'|'APPROVED'|'REJECTED'; goodQuantity:number; scrapQuantity:number; defectCode?:string; comment?:string; }
export interface ProductionEvent { id:string; taskId?:string; type:'TASK_STARTED'|'TASK_PAUSED'|'TASK_RESUMED'|'TASK_BLOCKED'|'TASK_COMPLETED'|'RESULT_RECORDED'|'DOWNTIME_STARTED'|'DOWNTIME_ENDED'|'MAINTENANCE_STARTED'|'MAINTENANCE_COMPLETED'; occurredAt:string; actorId:string; payload:Record<string,unknown>; }

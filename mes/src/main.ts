...
const working = calendar.filter(d => d.isWorking);
const employeeSchedules = [
  ...working.map(d => ({ employeeId: 'E-001', date: d.date, shiftIds: ['SHIFT-DAY'], status: 'WORK' as const })),
  ...working.map(d => ({ employeeId: 'E-002', date: d.date, shiftIds: ['SHIFT-DAY'], status: 'WORK' as const })),
  ...working.map(d => ({ employeeId: 'E-003', date: d.date, shiftIds: ['SHIFT-NIGHT'], status: 'WORK' as const }))
];
...
const seed: MesState = {
  ...
  shifts, calendar, employeeSchedules,
...
function ensureHorizon(): void {
  ...
  const end = start + HORIZON_DAYS * DAY_MS;
  if (new Date(state.plan.horizonEnd).getTime() >= end && state.calendar.length >= HORIZON_DAYS) return;
  state.plan.horizonEnd = new Date(end).toISOString();
...
function calculate(): void {
  state.tasks = buildDeterministicSchedule({ orders: state.orders, employees: state.employees, equipment: state.equipment, horizonStart: state.plan.horizonStart, horizonEnd: state.plan.horizonEnd, shifts: state.shifts, calendar: state.calendar, employeeSchedules: state.employeeSchedules, equipmentBlocks: state.equipmentBlocks }).tasks;
  saveState(state);
}
...
function errorBox(error: unknown): void { window.alert(error instanceof Error ? error.message : 'Операция не выполнена'); }
...
function render(): void {
  ...
  const operations = state.orders.flatMap(o => o.route);
  root.innerHTML = `<header class="topbar"><div><div class="eyebrow">ТЕХНОХАБ ЗСМК</div><h1>MES • Производственное управление</h1></div><div class="plan-badge">План v${state.plan.version} · ${state.plan.status}</div></header><main class="page"><section class="kpis"><article><span>Заказы</span><strong>${state.orders.length}</strong></article><article><span>Задания</span><strong>${state.tasks.length}</strong></article><article><span>Выпущено</span><strong>${totalGood}</strong></article><article class="${conflicts.length ? 'danger' : ''}"><span>Конфликты</span><strong>${conflicts.length}</strong></article><article><span>Открытые простои</span><strong>${openDowntime}</strong></article></section>
  <section class="panel"><div class="panel-head"><div><h2>Диспетчерская доска</h2><div class="subtle">30 дней · ${state.shifts.length} смены · ${state.equipmentBlocks.length} блокировки</div></div><button id="recalc" class="primary">Пересчитать</button></div>${renderDispatchBoard({ tasks: state.tasks, employees: state.employees, equipment: state.equipment, shifts: state.shifts, calendar: state.calendar, equipmentBlocks: state.equipmentBlocks, operations, onMove: moveTask, onAssignEmployee: (id, employeeId) => updateTask(id, { assignedEmployeeIds: employeeId ? [employeeId] : [] }), onAssignEquipment: (id, equipmentId) => updateTask(id, { assignedEquipmentIds: equipmentId ? [equipmentId] : [] })})}</section>
  ${renderExecutionPanel({ tasks: state.tasks, employees: state.employees, equipment: state.equipment, results: state.results, downtimes: state.downtimes, onAction: (id, action) => { try { executeTaskAction(state, id, action, actorId); saveState(state); render(); } catch (e) { errorBox(e); } }, onResult: (id, good, scrap, comment) => { try { const task = state.tasks.find(t => t.id === id); recordProductionResult(state, id, { goodQuantity: good, scrapQuantity: scrap, comment, employeeIds: task?.assignedEmployeeIds ?? [], equipmentIds: task?.assignedEquipmentIds ?? [] }, actorId); saveState(state); render(); } catch (e) { errorBox(e); } }, onDowntimeStart: (equipmentId, reasonCode, comment) => { try { startDowntime(state, { equipmentId, reasonCode, comment }, actorId); saveState(state); render(); } catch (e) { errorBox(e); } }, onDowntimeEnd: id => { try { endDowntime(state, id, actorId); saveState(state); render(); } catch (e) { errorBox(e); } })}
  <section class="panel">${renderCalendarEditor({ calendar: state.calendar, shifts: state.shifts, employees: state.employees, employeeSchedules: state.employeeSchedules, equipment: state.equipment, equipmentBlocks: state.equipmentBlocks, onCalendarChange: updateCalendarDay, onEmployeeScheduleChange: updateEmployeeSchedule, onAddBlock: addEquipmentBlock, onRemoveBlock: removeEquipmentBlock })}</section>
  <section class="grid-2"><div class="panel"><div class="panel-head"><h2>Заказы</h2></div><table><thead><tr><th>Заказ</th><th>Приоритет</th><th>Срок</th><th>Статус</th></tr></thead><tbody>${state.orders.map(o => `<tr><td><strong>${o.number}</strong></td><td>${o.priority}</td><td>${new Date(o.dueAt).toLocaleDateString('ru-RU')}</td><td>${o.status}</td></tr>`).join('')}</tbody></table></div><div class="panel"><div class="panel-head"><h2>Задания</h2></div><table><thead><tr><th>Задание</th><th>Операция</th><th>Интервал</th><th>Сотрудник</th><th>Оборудование</th><th>Версия</th></tr></thead><tbody>${state.tasks.map(t => { const op = operations.find(o => o.id === t.operationId); const e = state.employees.find(x => x.id === t.assignedEmployeeIds[0]); const eq = state.equipment.find(x => x.id === t.assignedEquipmentIds[0]); return `<tr><td>${t.id}</td><td>${op?.name ?? t.operationId}</td><td>${new Date(t.plannedStart).toLocaleString('ru-RU')} → ${new Date(t.plannedEnd).toLocaleString('ru-RU')}</td><td>${e?.name ?? '—'}</td><td>${eq?.name ?? '—'}</td><td>v${t.version}</td></tr>`; }).join('')}</tbody></table></div></section></main>`;
  root.querySelector<HTMLButtonElement>('#recalc')?.addEventListener('click', () => { state.tasks = []; calculate(); render(); });
  root.querySelectorAll<HTMLButtonElement>('[data-move]').forEach(b => b.addEventListener('click', () => moveTask(b.dataset.move ?? '', Number(b.dataset.delta ?? 0))));
  root.querySelectorAll<HTMLSelectElement>('[data-employee]').forEach(s => s.addEventListener('change', () => updateTask(s.dataset.employee ?? '', { assignedEmployeeIds: s.value ? [s.value] : [] })));
  root.querySelectorAll<HTMLSelectElement>('[data-equipment]').forEach(s => s.addEventListener('change', () => updateTask(s.dataset.equipment ?? '', { assignedEquipmentIds: s.value ? [s.value] : [] })));

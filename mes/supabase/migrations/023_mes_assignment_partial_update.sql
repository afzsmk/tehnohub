-- Partial assignment contract for MES.
-- NULL means "leave this resource dimension unchanged"; an explicit JSON array
-- replaces that dimension. This keeps employee and equipment selections independent.
...
begin
  ...
  if exists (
    select 1 from jsonb_array_elements(coalesce(p_employee_ids, '[]'::jsonb)) item
    where jsonb_typeof(item) <> 'string'
  ) or exists (
    select 1 from jsonb_array_elements(coalesce(p_equipment_ids, '[]'::jsonb)) item
    where jsonb_typeof(item) <> 'string'
  ) then
  ...
  select o.product_id into v_product_id
    from production_orders o where o.id = v_task.order_id;
  ...
  foreach v_employee_id in array v_unique_employees loop
    ...
    if v_required_qualification is not null and not exists (
      select 1 from employees
       where id = v_employee_id and active and qualification_level >= v_required_qualification
    ) then
  ...
  foreach v_equipment_id in array v_unique_equipment loop
    ...
    if exists (
      select 1 from equipment e
       where e.id = v_equipment_id and e.active and e.work_center <> v_operation_work_center
    ) then
  ...
  if exists (
    ...
    where not exists (
      select 1 from jsonb_array_elements_text(v_effective_equipment_ids) selected_id
      where selected_id = required_id
    )
  ) then

...
begin
    ...
    select
      ...
      (greatest(0, e.qualification_level - coalesce(v_required_qualification, 0)) * 10
       + (select count(*) from task_assignments a join production_tasks t on t.id = a.task_id
           where a.employee_id = e.id and t.status in ('ASSIGNED','READY','RUNNING','PAUSED','PARTIALLY_COMPLETED')
             and t.id <> v_task.id and tstzrange(t.planned_start,t.planned_end,'[)') && v_work_range))::numeric as score,
      jsonb_build_array(
        format('Квалификация: %s', e.qualification_level),
        'Сотрудник активен',
        'Нет пересечения по заданиям',
        'Календарь/смена покрывают весь интервал'
      ) as reasons
    from employees e
    where e.active
      ...
      and exists (
        ...
        where cd.is_working
          ...
          and exists (
            select 1 from shift_definitions s
            where s.active
        ...
        group by e.id
        having range_agg(tstzrange(
          ((cd.date::text || ' 00:00:00+00')::timestamptz) + (s.start_minute * interval '1 minute'),
          ((cd.date::text || ' 00:00:00+00')::timestamptz) + ((s.start_minute + s.duration_minutes) * interval '1 minute'),'[)')) @> v_work_range
      )
  )
  select 'EMPLOYEE', id, name, score, reasons from employee_candidates order by score, name
  limit 20;
  ...
  with equipment_candidates as (
    select
      ...
      e.name,
      (select count(*) from task_assignments a join production_tasks t on t.id=a.task_id
       where a.equipment_id=e.id and t.status in ('ASSIGNED','READY','RUNNING','PAUSED','PARTIALLY_COMPLETED')
         and t.id <> v_task.id and tstzrange(t.planned_start,t.planned_end,'[)') && v_work_range)::numeric as score,
      jsonb_build_array(
        format('Участок: %s', e.work_center),
    ...
    from equipment e
    where e.active
      and e.work_center = v_work_center
      and (
      ...
      and (
        jsonb_array_length(coalesce(e.capabilities,'[]'::jsonb)) = 0
        or e.capabilities ? v_operation_code
        or e.capabilities ? v_work_center
      )
      and not exists (
      ...
      and not exists (
        select 1 from equipment_blocks b where b.equipment_id=e.id
          and tstzrange(b.start_at,b.end_at,'[)') && v_work_range
      )
      and not exists (
        select 1 from maintenance_orders m where m.equipment_id=e.id
          and m.status in ('PLANNED','IN_PROGRESS')
          and tstzrange(m.planned_start,m.planned_end,'[)') && v_work_range
      )
  )
  select 'EQUIPMENT', id, name, score, reasons from equipment_candidates order by score, name
  limit 20;
end;
$$;

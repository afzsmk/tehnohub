...
begin
  ...
  for r in select value from jsonb_array_elements(coalesce(p_payload->'equipment','[]'::jsonb)) loop
    if nullif(trim(r->>'work_center'),'') is not null then
      select coalesce(id,'') into v_wc
        from work_centers
       where (id=trim(r->>'work_center') or code=trim(r->>'work_center'))
         and active=true
       limit 1;
      if v_wc is null or v_wc='' then
        if not exists(
          select 1 from jsonb_array_elements(coalesce(p_payload->'work_centers','[]'::jsonb)) wc_json
  ...
  for r in select value from jsonb_array_elements(coalesce(p_payload->'route_operations','[]'::jsonb)) loop
    ...
    if nullif(trim(r->>'route_id'),'') is not null then
      ...
      if v_route_product is null then
        select trim(route_json->>'product_id') into v_route_product
          from jsonb_array_elements(coalesce(p_payload->'routes','[]'::jsonb)) route_json
         where trim(route_json->>'id')=trim(r->>'route_id') and coalesce((route_json->>'active')::boolean,true)
         limit 1;
      end if;
      if v_route_product is null then
    ...
    if nullif(trim(r->>'work_center'),'') is not null then
      select id into v_wc from work_centers
       where (id=trim(r->>'work_center') or code=trim(r->>'work_center')) and active=true limit 1;
      if v_wc is null then
        select trim(wc_json->>'id') into v_wc
          from jsonb_array_elements(coalesce(p_payload->'work_centers','[]'::jsonb)) wc_json
         where (trim(wc_json->>'id')=trim(r->>'work_center') or trim(wc_json->>'code')=trim(r->>'work_center'))
    ...
    if nullif(trim(r->>'required_qualification_id'),'') is not null then
      select id into v_qual from qualification_levels
       where (id=trim(r->>'required_qualification_id') or code=trim(r->>'required_qualification_id')) and active=true limit 1;
      if v_qual is null then
        select trim(qual_json->>'id') into v_qual
          from jsonb_array_elements(coalesce(p_payload->'qualification_levels','[]'::jsonb)) qual_json
    ...
    if jsonb_typeof(coalesce(r->'required_equipment_ids','[]'::jsonb))<>'array' then
      ...
      for x in select value from jsonb_array_elements(coalesce(r->'required_equipment_ids','[]'::jsonb)) loop
        if jsonb_typeof(x)<>'string' then
          ...
          if v_equipment_id='' then continue; end if;
          v_equipment_wc:=null;
          v_equipment_wc_id:=null;
          select e.work_center,e.work_center_id into v_equipment_wc,v_equipment_wc_id
            from equipment e where e.id=v_equipment_id and e.active limit 1;
          if v_equipment_wc is null and v_equipment_wc_id is null then
            select coalesce(ej->>'work_center',''),coalesce(ej->>'work_center_id','') into v_equipment_wc,v_equipment_wc_id
              from jsonb_array_elements(coalesce(p_payload->'equipment','[]'::jsonb)) ej
             where trim(ej->>'id')=v_equipment_id and coalesce((ej->>'active')::boolean,true) limit 1;
          end if;
          if v_equipment_wc is null and v_equipment_wc_id is null then
            ...
            elsif nullif(trim(v_equipment_wc),'') is not null
              and not exists(select 1 from work_centers w where w.id=v_wc and (w.name=trim(v_equipment_wc) or w.code=trim(v_equipment_wc))) then
              v_errors:=v_errors||jsonb_build_array(format('Route operation %s: оборудование %s относится к другому рабочему центру',r->>'id',v_equipment_id));
            end if;
          end if;
        end if;

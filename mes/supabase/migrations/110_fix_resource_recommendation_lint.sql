-- Fix SQL-lint false positives/ambiguity without editing already-applied migrations.
-- The returned table exposes an output column named `score`, while the CTE also
-- defines a column with the same name. Qualify the CTE columns explicitly.
...
begin
      ...
      (greatest(0, e.qualification_level - coalesce(v_required_qualification,0)) * 10
       + (select count(*) from task_assignments a join production_tasks t on t.id=a.task_id
          where a.employee_id=e.id and t.id<>v_task.id
            and t.status in ('ASSIGNED','READY','RUNNING','PAUSED','PARTIALLY_COMPLETED')))::numeric as candidate_score,
      jsonb_build_array(format('Квалификация: %s',e.qualification_level),'Сотрудник активен','Нет пересечения по заданиям','Календарь/смена покрывают весь интервал') as candidate_reasons
    from employees e
    join lateral (
  ...
  select 'EMPLOYEE', ec.id, ec.name, ec.candidate_score, ec.candidate_reasons
   ...
   order by ec.candidate_score, ec.name
   limit 20;
    ...
    select e.id, e.name,
      (select count(*) from task_assignments a join production_tasks t on t.id=a.task_id
       where a.equipment_id=e.id and t.id<>v_task.id
         and t.status in ('ASSIGNED','READY','RUNNING','PAUSED','PARTIALLY_COMPLETED')
         and tstzrange(t.planned_start,t.planned_end,'[)') && v_work_range)::numeric as candidate_score,
      jsonb_build_array(format('Участок: %s',e.work_center),'Оборудование активно','Нет пересечения по заданиям','Нет блока или обслуживания на интервале') as candidate_reasons
    from equipment e

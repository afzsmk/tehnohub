-- Serialize task assignment against server-side replanning for the same operational plan.
-- mes_apply_replan already locks operational_plans before validating changes.
-- This version of mes_assign_task acquires the same plan lock before locking the task,
-- so assignment cannot change resources after replan has validated them.
...
begin
  ...
  -- Lock the operational plan first, matching mes_apply_replan lock order.
  select o.plan_id
    into v_plan_id
  ...
  -- Transaction-scoped advisory locks close races between assignments on resources.

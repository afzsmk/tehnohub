-- Harden the database boundary for shop-floor execution.
-- UI may request an action, but the database is authoritative for the
-- production-task state machine and resource evidence.
...
begin
  ...
  if not can_transition_status(v_task.status, v_next_status) then
    raise exception 'Недопустимый переход задания: % -> %', v_task.status, v_next_status;
  end if;
...
begin
  ...
  if jsonb_typeof(coalesce(p_equipment_ids, '[]'::jsonb)) <> 'array'
     or exists (
       select 1
       from jsonb_array_elements(coalesce(p_equipment_ids, '[]'::jsonb)) x
  ...
  update production_tasks
     set actual_quantity = v_new_actual,
         status = case
           when v_new_actual >= planned_quantity
             and coalesce(quality_required, false)
             and coalesce(quality_status, 'NOT_REQUIRED') <> 'APPROVED'
             then 'PARTIALLY_COMPLETED'
           when can_transition_status(status, v_next_status) then v_next_status
           else status
         end,
         actual_end = case
           when v_new_actual >= planned_quantity
             and not (coalesce(quality_required, false)
                      and coalesce(quality_status, 'NOT_REQUIRED') <> 'APPROVED')
             then coalesce(actual_end, p_recorded_at)
           else actual_end
         end,
         quality_status = case
           when v_new_actual > v_task.actual_quantity
             and coalesce(quality_required, false)
             then 'PENDING'
           else quality_status
         end,

-- Harden normalized bootstrap validation before topology-aware writes.
-- Normalized references must point to active entities and route-operation product
-- identity must match the selected route header.
  ...
  for r in select value from jsonb_array_elements(coalesce(p_payload->'equipment','[]'::jsonb)) loop
    if nullif(trim(r->>'work_center'),'') is not null then
      ...
      if v_wc is null or v_wc='' then
        if not exists(select 1 from jsonb_array_elements(coalesce(p_payload->'work_centers','[]'::jsonb)) x where (trim(x->>'id')=trim(r->>'work_center') or trim(x->>'code')=trim(r->>'work_center')) and coalesce((x->>'active')::boolean,true)) then
  ...
  for r in select value from jsonb_array_elements(coalesce(p_payload->'route_operations','[]'::jsonb)) loop
    if nullif(trim(r->>'route_id'),'') is not null then
      ...
      if v_route_product is null then
        select trim(x->>'product_id') into v_route_product from jsonb_array_elements(coalesce(p_payload->'routes','[]'::jsonb)) x where trim(x->>'id')=trim(r->>'route_id') and coalesce((x->>'active')::boolean,true) limit 1;
      end if;
      if v_route_product is null then
    ...
    if nullif(trim(r->>'work_center'),'') is not null then
      ...
      if v_wc is null then
        select trim(x->>'id') into v_wc from jsonb_array_elements(coalesce(p_payload->'work_centers','[]'::jsonb)) x where (trim(x->>'id')=trim(r->>'work_center') or trim(x->>'code')=trim(r->>'work_center')) and coalesce((x->>'active')::boolean,true) limit 1;
      end if;
      if v_wc is null then
    ...
    if nullif(trim(r->>'required_qualification_id'),'') is not null then
      ...
      if v_qual is null then
        select trim(x->>'id') into v_qual from jsonb_array_elements(coalesce(p_payload->'qualification_levels','[]'::jsonb)) x where (trim(x->>'id')=trim(r->>'required_qualification_id') or trim(x->>'code')=trim(r->>'required_qualification_id')) and coalesce((x->>'active')::boolean,true) limit 1;
      end if;
      if v_qual is null then
    ...
    for x in select value from jsonb_array_elements(coalesce(r->'required_equipment_ids','[]'::jsonb)) loop
      ...
      if v_equipment_id<>'' and not exists(select 1 from equipment where id=v_equipment_id and active=true)
         and not exists(select 1 from jsonb_array_elements(coalesce(p_payload->'equipment','[]'::jsonb)) e where trim(e->>'id')=v_equipment_id and coalesce((e->>'active')::boolean,true)) then

-- Harden the production-event outbox trigger so maintenance events carry
-- the MES plan context emitted by the application.
...
begin
  ...
  -- Task-linked events derive plan/order/product context from the relational graph.
  ...
  -- Maintenance events have no task_id, so use the plan context placed into payload
  -- by the MES application layer.
  v_plan_id := coalesce(v_plan_id, NEW.payload->>'mesPlanId');
  v_plan_version := coalesce((NEW.payload->>'mesPlanVersion')::integer, null);
  ...
  if v_plan_version is null then

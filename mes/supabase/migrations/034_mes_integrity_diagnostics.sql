-- MES operational integrity diagnostics.
-- Read-only diagnostic RPC for detecting impossible or inconsistent states
-- across orders, tasks, resources and the Quality Gate.
...
begin
  ...
  with violations as (
    select t.id as entity_id,
           'TASK_OVER_PLAN'::text as code,
           'MES_PRODUCTION_TASK'::text as entity_type,
           format('Факт задания %s превышает план %s', t.actual_quantity, t.planned_quantity) as message
    ...
    select o.id,
      ...
      from production_orders o
      join lateral (
        select coalesce(sum(t.actual_quantity), 0) as latest_quantity
          from production_tasks t
    ...
    select t.id,
     ...
     where t.status = 'COMPLETED'
       and t.quality_required
       and t.quality_status <> 'APPROVED'

    union all

    select a.task_id,
           'INACTIVE_EMPLOYEE_ASSIGNMENT',
           'MES_TASK_ASSIGNMENT',
...
'Read-only MES integrity diagnostics across task facts, order facts, resources and Quality Gate.';

-- Refine assignment RPC so a dispatcher can change one resource dimension
-- without clearing the other. NULL means preserve the current assignment set;
-- an empty JSON array explicitly clears that resource dimension.
...
begin
  ...
  select coalesce(array_agg(distinct a.employee_id order by a.employee_id) filter (where a.employee_id is not null), '{}'::text[]),
         coalesce(array_agg(distinct a.equipment_id order by a.equipment_id) filter (where a.equipment_id is not null), '{}'::text[])
    into v_current_employees, v_current_equipment
    from task_assignments a
   where a.task_id = p_task_id;

  if p_employee_ids is null then
    v_unique_employees := v_current_employees;
  else
    if jsonb_typeof(p_employee_ids) <> 'array' then raise exception 'employeeIds должен быть массивом'; end if;
    select coalesce(array_agg(distinct value order by value), '{}'::text[])
      into v_unique_employees
      from jsonb_array_elements_text(p_employee_ids) as value
     where trim(value) <> '';
  end if;

  if p_equipment_ids is null then
    v_unique_equipment := v_current_equipment;
  else
    if jsonb_typeof(p_equipment_ids) <> 'array' then raise exception 'equipmentIds должен быть массивом'; end if;
    select coalesce(array_agg(distinct value order by value), '{}'::text[])
  ...
  v_before := jsonb_build_object(
    ...
    'employeeIds', to_jsonb(v_current_employees),
    'equipmentIds', to_jsonb(v_current_equipment)
  );

-- MES order completion hardening.
-- Manual order completion must use the latest technological operation,
-- matching mes_sync_order_from_task, instead of summing all operations.
...
begin
  ...
  if p_next_status = 'PLANNED' then
    ...
    if v_latest_sequence is null then
      raise exception 'У заказа нет действующих производственных заданий';
    end if;
  ...
  update production_orders
     set status = p_next_status,
         completed_quantity = case
           when p_next_status in ('PARTIALLY_COMPLETED','COMPLETED') then greatest(completed_quantity, least(v_order.quantity, v_next_completed))
...
'Controlled MES production-order lifecycle; completion facts are derived from the latest technological operation.';

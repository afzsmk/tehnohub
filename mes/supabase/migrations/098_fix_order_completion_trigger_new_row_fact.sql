-- Validate order completion against the row being written, not the persisted
-- OLD row visible from a BEFORE UPDATE trigger.
--
-- mes_assert_order_route_completeness(text, text) intentionally remains the
-- public compatibility entry point and reads the current persisted order.
-- The status trigger additionally uses an internal value-aware validator so a
-- single atomic update of completed_quantity + status cannot validate against
-- the previous completed_quantity.

-- Complete the bootstrap import contract for normalized work centers and route headers.
-- Existing route_operations/product/work_center text fields remain as compatibility data;
-- new normalized FKs are populated during import.

create or replace function mes_validate_bootstrap_topology(p_payload jsonb)
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_errors jsonb:='[]'::jsonb; r jsonb; v_id text;
begin
  ...
  for r in select value from jsonb_array_elements(coalesce(p_payload->'routes','[]'::jsonb)) loop
    ...
    if not exists(select 1 from jsonb_array_elements(coalesce(p_payload->'products','[]'::jsonb)) x where trim(x->>'id')=trim(r->>'product_id'))
  ...
  for r in select value from jsonb_array_elements(coalesce(p_payload->'equipment','[]'::jsonb)) loop
    if nullif(trim(r->>'work_center'),'') is not null
       and not exists(select 1 from jsonb_array_elements(coalesce(p_payload->'work_centers','[]'::jsonb)) x where trim(x->>'id')=trim(r->>'work_center') or trim(x->>'code')=trim(r->>'work_center'))
       and not exists(select 1 from work_centers where id=trim(r->>'work_center') or code=trim(r->>'work_center')) then
      v_errors:=v_errors||jsonb_build_array(format('Equipment %s: work_center %s отсутствует',r->>'id',r->>'work_center'));
    end if;
  end loop;

  for r in select value from jsonb_array_elements(coalesce(p_payload->'route_operations','[]'::jsonb)) loop
    if nullif(trim(r->>'route_id'),'') is not null
       and not exists(select 1 from jsonb_array_elements(coalesce(p_payload->'routes','[]'::jsonb)) x where trim(x->>'id')=trim(r->>'route_id'))
       and not exists(select 1 from routes where id=trim(r->>'route_id')) then
      v_errors:=v_errors||jsonb_build_array(format('Route operation %s: route %s отсутствует',r->>'id',r->>'route_id'));
    end if;
    if nullif(trim(r->>'work_center'),'') is not null
       and not exists(select 1 from jsonb_array_elements(coalesce(p_payload->'work_centers','[]'::jsonb)) x where trim(x->>'id')=trim(r->>'work_center') or trim(x->>'code')=trim(r->>'work_center'))
       and not exists(select 1 from work_centers where id=trim(r->>'work_center') or code=trim(r->>'work_center')) then
      v_errors:=v_errors||jsonb_build_array(format('Route operation %s: work_center %s отсутствует',r->>'id',r->>'work_center'));
    end if;
  end loop;
...
begin
  ...
  begin
    ...
    n:=0; for r in select value from jsonb_array_elements(coalesce(p_payload->'route_operations','[]'::jsonb)) loop v_route:=null; if nullif(trim(r->>'route_id'),'') is not null then select id into v_route from routes where id=trim(r->>'route_id') limit 1; end if; v_wc:=null; select id into v_wc from work_centers where id=trim(r->>'work_center') or code=trim(r->>'work_center') limit 1; perform mes_master_save_route_operation(r->>'id',r->>'product_id',coalesce((r->>'sequence')::integer,0),r->>'code',r->>'name',coalesce(v_wc,r->>'work_center'),(r->>'required_qualification')::integer,coalesce(r->'required_equipment_ids','[]'::jsonb),coalesce((r->>'setup_norm_hours')::numeric,0),coalesce((r->>'labor_norm_hours_per_unit')::numeric,0),coalesce((r->>'workers_required')::integer,1),coalesce((r->>'active')::boolean,true)); update route_operations set route_id=v_route,work_center_id=v_wc where id=r->>'id'; n:=n+1; end loop; v_counts:=jsonb_set(v_counts,'{route_operations}',to_jsonb(n),true);
...
-- New privileged RPCs must remain browser-authenticated only; anonymous callers are denied.
revoke execute on function mes_validate_bootstrap_topology(jsonb) from public, anon;
revoke execute on function mes_master_save_work_center(text,text,text,text,text,text,boolean) from public, anon;
revoke execute on function mes_master_save_route(text,text,text,text,text,integer,boolean,date,date,text) from public, anon;

-- Reopen the quality gate whenever a new production fact is appended to a
-- quality-controlled task. A prior APPROVED inspection applies only to the
-- production quantity it inspected; a later fact must wait for a new approval.
-- Non-quality-controlled tasks retain their existing automatic completion path.
...
begin
  ...
  -- Quality approval is scoped to the facts known at the time of approval.
  -- Any subsequent production fact on a quality-controlled task reopens the gate,
  -- including the case where the new accumulated quantity reaches the plan.
  update production_tasks
     set actual_quantity = v_new_actual,
         status = case
           when not coalesce(quality_required, false)
            and v_new_actual >= planned_quantity then 'COMPLETED'
           else 'PARTIALLY_COMPLETED'
         end,
         actual_end = case
           when not coalesce(quality_required, false)
            and v_new_actual >= planned_quantity then coalesce(actual_end, p_recorded_at)
           else null
         end,
         quality_status = case
           when coalesce(quality_required, false) then 'PENDING'
           else quality_status
         end,
         version = version + 1
   where id = p_task_id;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'qualityGateReopened', coalesce(v_task.quality_required, false),

-- Equipment blocks use half-open [start_at, end_at) intervals everywhere else in MES.
-- START must therefore reject the exact block start instant as unavailable.
...
begin
    ...
    loop
      ...
      if not exists (
        select 1 from equipment e
         where e.id = v_equipment_id and e.active
      ) then
  ...
  select max(e.occurred_at) into v_last_event_at
    from production_events e where e.task_id = p_task_id;
  ...
  values (
    ...
    p_task_id, v_event_type, p_occurred_at, auth.uid()::text,

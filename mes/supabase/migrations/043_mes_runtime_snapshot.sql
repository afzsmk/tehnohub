...
begin
  ...
  if v_role = '' then
    raise exception 'MES role is not configured';
  end if;
  ...
  if v_plan.id is null then
    return jsonb_build_object(
      ...
      'shifts', '[]'::jsonb,
      'calendar', '[]'::jsonb,
      'employeeSchedules', '[]'::jsonb,
      'equipmentBlocks', '[]'::jsonb,
      'orders', '[]'::jsonb,
      'tasks', '[]'::jsonb,
      'downtimes', '[]'::jsonb,
      'maintenance', '[]'::jsonb,
      'results', '[]'::jsonb,
      'qualityInspections', '[]'::jsonb,
      'events', '[]'::jsonb
    );
  end if;
  ...
  return jsonb_build_object(
    'plan', jsonb_build_object(
      ...
      'horizonStart', v_plan.horizon_start,
      'horizonEnd', v_plan.horizon_end,
      'status', v_plan.status,
      'sourcePlanId', v_plan.source_plan_id,
      'sourcePlanVersion', v_plan.source_plan_version
    ),
    'products', coalesce((select jsonb_agg(to_jsonb(p) order by p.id) from products p), '[]'::jsonb),
    'employees', coalesce((select jsonb_agg(to_jsonb(e) order by e.id) from employees e), '[]'::jsonb),
    'equipment', coalesce((select jsonb_agg(to_jsonb(e) order by e.id) from equipment e), '[]'::jsonb),
    ...
    'orders', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'quantity',o.quantity,
      'completedQuantity',o.completed_quantity,
      'dueAt',o.due_at,
      'priority',o.priority,
      'status',o.status,
      'route',coalesce((select jsonb_agg(jsonb_build_object(
        ...
        'code',ro.code,
        'name',ro.name,
        'workCenter',ro.work_center,
        'requiredQualification',ro.required_qualification,
        'requiredEquipmentIds',ro.required_equipment_ids,
        'setupMinutes',ro.setup_minutes,
        'runMinutesPerUnit',ro.run_minutes_per_unit
      ) order by ro.sequence) from route_operations ro where ro.product_id=o.product_id and ro.active), '[]'::jsonb)
    ) order by o.due_at,o.priority desc,o.id) from production_orders o where o.plan_id=v_plan.id), '[]'::jsonb),
    'tasks', coalesce((select jsonb_agg(jsonb_build_object(
      'id',t.id,
      'orderId',t.order_id,
      'operationId',t.operation_id,
      'operationSequence',t.operation_sequence,
      'status',t.status,
      'plannedStart',t.planned_start,
      'plannedEnd',t.planned_end,
      'actualStart',t.actual_start,
      'actualEnd',t.actual_end,
      'plannedQuantity',t.planned_quantity,
      'actualQuantity',t.actual_quantity,
      'assignedEmployeeIds',coalesce((select jsonb_agg(a.employee_id order by a.employee_id) from task_assignments a where a.task_id=t.id and a.employee_id is not null), '[]'::jsonb),
      'assignedEquipmentIds',coalesce((select jsonb_agg(a.equipment_id order by a.equipment_id) from task_assignments a where a.task_id=t.id and a.equipment_id is not null), '[]'::jsonb),
      'qualityRequired',t.quality_required,
      'qualityStatus',t.quality_status,
      'version',t.version
    ) order by t.planned_start,t.operation_sequence,t.id) from production_tasks t join production_orders o on o.id=t.order_id where o.plan_id=v_plan.id), '[]'::jsonb),
    'downtimes', coalesce((select jsonb_agg(jsonb_build_object('id',d.id,'equipmentId',d.equipment_id,'reasonCode',d.reason_code,'startedAt',d.started_at,'endedAt',d.ended_at,'comment',d.comment) order by d.started_at,d.id) from downtime_events d where d.equipment_id in (select id from equipment)), '[]'::jsonb),

...
values (jsonb_build_object(
  ...
  'employees', jsonb_build_array(
    jsonb_build_object('id','BOOT-EMP-1','personnel_no','BOOT-001','name','Bootstrap Worker','profession','BOOT-OP','profession_id','BOOT-PROF-1','brigade_id','BOOT-BR-1','qualification_id','BOOT-QUAL-1','qualification_level',3,'active',true)
  ),
...
select is((select count(*) from shifts where id='BOOT-SHIFT-1'),1::bigint,'bootstrap imports shift');
...
-- Repeated bootstrap delivery must upsert, not duplicate normalized master data.
